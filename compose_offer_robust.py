...
def coerce_student_offer_from_config(config: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
    ...
    for course in courses_config:
        if isinstance(course, dict):
            course_entry = {
                ...
                "IntakeDate": to_iso8601(course.get('intake_date') or datetime.now(tz=AEST)),
                "StartDate": to_iso8601(course.get('start_date') or datetime.now(tz=AEST)+timedelta(days=10)),
    ...
    if not cleaned_courses:
        cleaned_courses = [{
            ...
            "IntakeDate": to_iso8601(datetime.now(tz=AEST)),
            "StartDate": to_iso8601(datetime.now(tz=AEST)+timedelta(days=10)),
